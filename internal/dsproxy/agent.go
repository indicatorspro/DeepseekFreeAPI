...
// conversation. Anti-loop anchor (mirrors the GLM-Free-API fix for the same
// drift): long agent sessions loop into re-issuing identical calls (or endless
// thinking) because the history replay never tells the model what is already
// done; an explicit, compact list plus the no-repeat rules above gives it a
// hard, current state to check against.
// Returns "" when no calls have been made yet (section omitted).
func agentAlreadyCalledList(messages []chatMessage) string {
	var lines []string
